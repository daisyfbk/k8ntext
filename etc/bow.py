...
parser = argparse.ArgumentParser(
    prog='parseLog',
    description='This program takes a LABELED log as input and transforms it into a BoW representation.'
...
def add_to_bow(line, label):
    ...
    for key in indexed_keys:
        value = line.get(key)
        if key == "objectRef":
            value = parse_objectRef(value)
        elif key == "user":
            value = parse_user(value)
        elif key == "requestURI":
            value = parse_uri(value)
        mapping[label][(key, value)] = mapping[label].get((key, value), 0) + 1
...
def main():
    ...
    output_filename = input_filename + "_bow"
