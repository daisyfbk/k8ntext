...
# Autoencoder for log clustering
...
def parse_objectRef(objectRef):
    try:
        return objectRef.get("resource", "None") + "/" + objectRef.get("namespace", "None") + "/" + objectRef.get("name", "None") + "/" + objectRef.get("subresource", "None")
    except:
        return None
...
def parse_user(user):
    if user is None:
        return None
    res = user.get("username", "None")   
    #if "groups" in user:
    #    res += "<"
    #    res += "/".join(user["groups"])
    #    res += ">"
    return res
...
parser = argparse.ArgumentParser(
    prog='parseLog',
    description='This program takes a LABELED log as input and trains a autoencoder model to cluster logs based on their content.'
)
...
def main():
    ...
    df["objectRef"] = df["objectRef"].apply(parse_objectRef)
    df["user"] = df["user"].apply(parse_user)
    df["requestURI"] = df["requestURI"].apply(parse_uri)
    df = df[["objectRef", "user", "requestURI"]]
    df = df.dropna()

    df = pd.get_dummies(df, columns=list(indexed_keys))
    ...
    encoded_data = pd.DataFrame(encoded_data, index=df.index)
    encoded_data = encoded_data.reset_index()
